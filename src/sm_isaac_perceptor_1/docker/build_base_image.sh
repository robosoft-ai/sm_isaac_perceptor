...
echo "---------------------------"
cd $ROOT_SRC_DIRECTORY/..
echo "building sm_isaac_perceptor_1 base image"
# build sm_isaac_perceptor_1 image
ARCH=$(uname -m)
echo "arch: $ARCH"
if [ $ARCH == "aarch64" ]; then
    sudo mount --bind / src/root_mount/
    docker build -t pibgeus/smacc2_isaac_exploration -f src/sm_isaac_perceptor_1/docker/Dockerfile_nvidia_arm64 --build-arg architecture_=$ARCH --progress=plain .
    sudo umount src/root_mount/
else
    docker build -t pibgeus/smacc2_isaac_exploration -f src/sm_isaac_perceptor_1/docker/Dockerfile --build-arg architecture_=$ARCH --progress=plain .
fi
